...
# Ideally, you would do the following steps
    ...
    #S2. Check the structure of the data - are all columns properly coded (numeric, categorical,...)
    str(dt)  #to show information about the structure of the data table
    
    #S3. Check for NAs and missing variables
...
# here is where efficiency comes in!
  ...
  #  Now  let's subset variables of interest - 
  dt.dp <- dt[,c("gender","age","local","city_size","education","part_time","job_satisfaction","kpi_performance","boss_survey","years_since_promotion","salary")]
  dt.e.dp <- dt.e[,c("gender","age","local","city_size","education","part_time","job_satisfaction","kpi_performance","boss_survey","years_since_promotion","salary")]
  dt.s.dp <- dt.s[,c("gender","age","local","city_size","education","part_time","job_satisfaction","kpi_performance","boss_survey","years_since_promotion","salary")]
...
#Q4C
  ...
  #Q4-ALL  OR you could have simply compared these together directly:
...
#Q5##########
  ...
  # Reg. demographic data between employees who exited and stayed show NO major differences
  #  -> if you highlighted some small differences, that is okay.  
  # in terms of employee climate:
  ...
  par(mfrow=c(2,3))    # set the plotting area into a 3*2 array
  boxplot(dt.e.dp$boss_survey,dt.s.dp$boss_survey,xlab="Boss Survey", names = c("Exit", "Remain"))
  boxplot(dt.e.dp$job_satisfaction,dt.s.dp$job_satisfaction,xlab="job Satisfaction", names = c("Exit", "Remain"))
  # additional plots to check intuition
  boxplot(dt.e.dp$kpi_performance,dt.s.dp$kpi_performance,xlab="Performance", names = c("Exit", "Remain"))
  boxplot(dt.e.dp$salary,dt.s.dp$salary,xlab="Salary", names = c("Exit", "Remain"))
  boxplot(dt.e.dp$years_since_promotion,dt.s.dp$years_since_promotion,xlab="years_since_promotion", names = c("Exit", "Remain"))
...
#Q8##########
  # Open question again.
  #I wanted to see how your framed the story and used the analysis you have so far.
  
  
#Q9##########
  
  # Looking to compute the following
  
  # OLS MODELS
  ModelA <- lm(exit ~ salary + rank +gender, data=dt)
  ModelB <- lm(exit ~ boss_survey + job_satisfaction, data=dt)
  ModelC <- lm(exit ~ boss_survey + job_satisfaction + salary + rank + gender, data=dt)
  
  # Let's compare the summary results from these OLS regression models
  stargazer(ModelA,ModelB,ModelC,type="text")
  AIC(ModelA,ModelB,ModelC)
  
# So Model 3 has adj r2 and lowest AIC. Model 3 is therefore the best
